...
Timestamp EPollPoller::poll(int timeoutMS, ChannelList *activeChannels)
{
    ...
    LOG_INFO("%s %d fd total count:%d\n", __FUNCTION__, __LINE__, (int)channels_.size());
    ...
    else
    {
        ...
        {
            errno = savedErrno;
            LOG_ERROR("%s %d EPollPoller::poll error", __FUNCTION__, __LINE__);
        }
    }
...
void EPollPoller::removeChannel(Channel *channel)
{
    ...
    LOG_INFO("%s %d fd=%d \n", __FUNCTION__, __LINE__, channel->fd());
...
void EPollPoller::update(int operation, Channel *channel)
{
    ...
    if (::epoll_ctl(epollfd_, operation, fd, &event) < 0)
    {
        ...
        {
            LOG_ERROR("%s %d epoll_ctr del error:%d\n", __FUNCTION__, __LINE__, errno);
        }
        else
        {
            LOG_FATAL("%s %d epoll_ctr add/mod error:%d\n", __FUNCTION__, __LINE__, errno);
        }
    }
}
