...
static EventLoop* CheckLoopNotNull(EventLoop* loop)
{
    if (loop == nullptr)
    {
        LOG_FATAL("%s %d TcpConnection is null.\n", __FUNCTION__, __LINE__);
    }
    return loop;
}
    ...
    highWaterMark_(64*1024*1024)
{
    ...
    LOG_INFO("%s %d TcpConnection::ctor[%s] at fd %d\n", __FUNCTION__, __LINE__, name.c_str(), sockfd);
    socket_->setKeepAlive(true);
}

TcpConnection::~TcpConnection()
{
    LOG_INFO("%s %d TcpConnection::dtor[%s] at fd %d state %d\n", __FUNCTION__, __LINE__, name_.c_str(), channel_->fd(), (int)state_);
}
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
    ...
    {
        errno = saveErrno;
        LOG_ERROR("%s %d TcpConnection handle error\n", __FUNCTION__, __LINE__);
        handleError();
    }
}
...
void TcpConnection::handleWrite()
{
    if (channel_->isWriting())
    {
        ...
        {
            LOG_ERROR("%s %d TcpConnection handleWrite\n", __FUNCTION__, __LINE__);
        }
    }
    else
    {
        LOG_ERROR("%s %d TcpConnection fd %d is down, no more writing\n", __FUNCTION__, __LINE__, channel_->fd());
    }
}

void TcpConnection::handleClose()
{
    LOG_INFO("%s %d fd %d state %d\n", __FUNCTION__, __LINE__, channel_->fd(), (int)state_);
...
void TcpConnection::handleError()
{
    ...
    LOG_ERROR("%s %d TcpConnection handle Error, name %s SO_ERROR %d\n", __FUNCTION__, __LINE__, name_.c_str(), err);
}
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    {
        LOG_ERROR("%s %d disconnected, give up writing.\n", __FUNCTION__, __LINE__);
        return ;
    }
