...
int createEventfd()
{
    ...
    if (evtfd < 0)
    {
        LOG_FATAL("%s %s %d eventfd error:%d", __FILENAME__, __FUNCTION__, __LINE__, errno);
    }
...
void EventLoop::handleRead()
{
  ...
  if (n != sizeof(one))
  {
    LOG_ERROR("%s %s %d reads %ld bytes instead of 8", __FILENAME__, __FUNCTION__, __LINE__, n);
  }
}
